...
variables 
    ...
    (* LWW Register state *)
    lww_value = [p \in Replica |-> 0]; \* current value of register
    lww_vc = [p \in Replica |-> vc_initial]; \* vc of last update
        ...
        while TRUE do 
            ...
            or
                ...
                with msg \in pending do
                    ...
                    (* --- LWW Register update --*)
                    \* we overwrite only when msg.vc is later or greater
                    if ~vc_leq(lww_vc[self.proc],msg.vc) then
                        lww_value[self.proc] := msg.msg;
                        lww_vc[self.proc] := msg.vc;
                    end if;
                    (* trigger rco-deliver *)
...
\* BEGIN TRANSLATION (chksum(pcal) = "e35eaebb" /\ chksum(tla) = "8e97afa5")
...
VARIABLES broadcast, delivered, happensBefore, available_messages, 
          rb_broadcast, rb_delivered, correctProcess, lww_value, lww_vc, 
          pending, VC
...
Init == (* Global variables *)
        ...
        /\ rb_delivered = [p \in Replica |-> EmptyBag]
        /\ correctProcess \in SUBSET Replica
        /\ lww_value = [p \in Replica |-> 0]
        /\ lww_vc = [p \in Replica |-> vc_initial]
...
client(self) == /\ \E msg \in available_messages:
                ...
                /\ UNCHANGED << delivered, rb_broadcast, rb_delivered, 
                                correctProcess, lww_value, lww_vc, pending, VC >>
...
causal_broadcast(self) == /\ \/ /\ \E msg \in broadcast[self.proc]:
                                ...
                                /\ UNCHANGED <<rb_delivered, lww_value, lww_vc, pending>>
                             \/ /\ \E msg \in BagToSet(rb_delivered[self.proc]):
                                ...
                                /\ UNCHANGED <<broadcast, delivered, rb_broadcast, lww_value, lww_vc, VC>>
                             \/ /\ \E msg \in pending[self]:
                                     /\ vc_leq(msg.vc, VC[self])
                                     /\ pending' = [pending EXCEPT ![self] = pending[self] \ {msg}]
                                     /\ IF ~vc_leq(lww_vc[self.proc],msg.vc)
                                           THEN /\ lww_value' = [lww_value EXCEPT ![self.proc] = msg.msg]
                                                /\ lww_vc' = [lww_vc EXCEPT ![self.proc] = msg.vc]
                                           ELSE /\ TRUE
                                                /\ UNCHANGED << lww_value, 
                                                                lww_vc >>
...
Convergence ==
    []( \A r1 \in Replica, r2 \in Replica :
          delivered[r1] = delivered[r2] => lww_value[r1] = lww_value[r2])
...
\* Last modified Tue Oct 14 10:32:31 CEST 2025 by tanvimoharir
\* Created Thu Sep 18 20:59:27 CEST 2025 by tanvimoharir
